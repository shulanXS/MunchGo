...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  const logoTarget = user?.role === 'MERCHANT' ? '/merchant'
      ...
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                ...
                <div className="relative">
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 mt-2 w-48 rounded-lg border bg-background shadow-lg py-1 z-50">
                      ...
                      {showCustomerNav && (
                        <>
                          <Link to="/profile" className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-muted" onClick={() => setUserMenuOpen(false)}>
                          ...
                          <Link to="/favorites" className="block px-4 py-2 text-sm hover:bg-muted" onClick={() => setUserMenuOpen(false)}>
                            我的收藏
                          </Link>
                      ...
                      {user?.role === 'RIDER' && (
                        <Link to="/rider" className="block px-4 py-2 text-sm hover:bg-muted" onClick={() => setUserMenuOpen(false)}>
                          骑手后台
                        </Link>
                      )}
                      {user?.role === 'ADMIN' && (
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t px-4 py-3 space-y-2">
            ...
            {!isAuthenticated && (
              <>
                <Link to="/login" className="block py-2 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>登录</Link>
                <Link to="/register" className="block py-2 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>注册</Link>
              </>
            )}
          </div>
        )}
